...
def correlate_probe_final_test(npi_df):
    ...
    # 1. 確保關鍵特徵欄位沒有空值，避免統計檢定崩潰
    clean_df = npi_df[["value", "yield_pct"]].dropna()

    # 2. 樣本數太少時防禦性回傳（Pearsonr 最少需要 2 筆，工業級建議至少 5 筆）
    if len(clean_df) < 5:
        return {
            "status": "Low_Sample",
            "pearson_r": 0.0,
