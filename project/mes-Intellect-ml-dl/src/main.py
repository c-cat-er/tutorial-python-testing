...
# 補齊專案三(classify_defects)、專案六(detect_unknown_defects)、專案七(train_domain_adaptation)、專案八(distill_teacher_to_student) 的完整引入
...
def main():
    ...
    # 執行專案三：ResNet18 晶圓缺陷分類
    logger.info("執行專案三：ResNet18 晶圓缺陷圖影像分類推論...")
    # 修改原本丟棄模型的底線「_」，正式用 resnet_teacher_model 變數接住實體，解決後面知識蒸餾的 NameError
    ...
    # 執行專案六：AutoEncoder/VAE 無監督未知缺陷識別
    ...
    # 執行專案七：跨機台/跨廠區 DaNN 知識遷移 (動態標記過濾重構版)
    logger.info("動態解析製程機台標記，啟動專案七：DaNN 領域自適應跨廠遷移訓練...")

    # 透過 MES 日誌或 NPI 資料中的設備代號，動態識別各晶圓屬於哪個廠區
    ...
    # 進行 Domain 雙域自動拆分 (無須拆分資料夾，從 DataFrame 直接切分矩陣)
    ...
    # 防禦性條件檢查：當確定產線資料中同時出現新、舊兩代不同廠區機台時，正式觸發對抗蒸餾
    ...
    # 聚合 MES 日誌：每片晶圓只抓「最大」的嚴重度（多列變 1 列）
    mes_agg = mes_clean.groupby(keys)[["severity"]].max().reset_index()

    # 聚合 NPI 資料：每片晶圓的量測值與良率取「平均值」（多列變 1 列）
    npi_agg = npi_result.groupby(keys)[["value", "yield_pct"]].mean().reset_index()

    # 三方欄位都是唯一「一片晶圓 1 列」，這時 merge 就絕對不會出錯
